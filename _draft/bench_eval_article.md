...
WhalePod 的上下文管理建立在一条核心断言上：**deepseek-v4 的自动前缀缓存能在长对话中持续生效**，前提是请求的字节前缀保持稳定。本报告从架构原理、评测系统设计和完整实验数据三个层面，系统性地验证这个断言。

报告涵盖两条互补轨道的完整流程：离线预测（`bench/validate.py`，零网络零 API key）和在线实测（`bench/live_acceptance.py`，真实服务器），以及两个维度的评价指标——可复用前缀份额（reusable prefix ratio）和命中率（cache hit rate）。全文用 60 个独立 session、三种对话长度（6/12/18 轮）和三种推理回放策略（never/tool/always）的数据来论证设计的稳健性。所有图表由 `bench/charts.py`（纯 stdlib）生成，可随时用 `python bench/article_charts.py` 复现。
...
### `bench/reasoning_compare.py` + `bench/article_charts.py` — 报告比较图

`reasoning_compare.py` 读取三份推理回放策略的 JSON（never/tool/always）渲染横向对比图（[§6.4](#64-推理回放策略-ab2026-08-11deepinfra-pin)）；`article_charts.py` 从已提交的 JSON 渲染全部跨 run 比较图——22-session 的 per-request 带宽、三种对话长度对比、MAE 曲线、累积成本结构（[§6.2](#62-在线实测-12-轮对话2026-08-06deepseek-官方-api)/[§6.3](#63-不同对话长度的对比)/[§6.5](#65-单-session-内的累积成本结构)）。两者都不需要网络和 API key。
...
### 3.4 Reasoning 控制回放 — 默认只保留工具回合的思考

DeepSeek 明确要求 chain-of-thought 不能无条件回放到后续请求中。WhalePod 的 `VLLMEndpoint._encode_message` 根据 `reasoning_strip` 决定是否把 `m.reasoning` 写回 payload（`whalepod/config.py:102`，默认 `"tool"`）：

- **never**：一律不写入 payload，只保留在本地 UI 中显示；
- **tool**（生产默认）：仅当该 assistant 回合调用了工具（`m.tool_calls`）时回放思考，纯回答回合照旧剥离；
- **always**：每个回合都回放。
...
- **[`reusable_prefix.svg`](../../whale-pod/bench/results/reusable_prefix.svg)** — 四种设计的 per-request 可复用前缀折线（1M 窗口）
- **[`reusable_prefix_small_window.svg`](../../whale-pod/bench/results/reusable_prefix_small_window.svg)** — 同上，45k 窗口（强迫 prune）
- **[`prompt_tokens_split.svg`](../../whale-pod/bench/results/prompt_tokens_split.svg)** — 四种设计的 prompt token 堆叠图（绿色 = 可复用，灰色 = 新鲜）
- **[`prompt_cost.svg`](../../whale-pod/bench/results/prompt_cost.svg)** — 四种设计的提示词成本对比
- **[`prompt_cost_small_window.svg`](../../whale-pod/bench/results/prompt_cost_small_window.svg)** — 同上，45k 窗口
- **[`prune_recovery.svg`](../../whale-pod/bench/results/prune_recovery.svg)** — as-built 在 45k 窗口下，prune 后前缀恢复曲线（▼ 标记 prune 点）
- **[`repo_map_budget.svg`](../../whale-pod/bench/results/repo_map_budget.svg)** — repo map 在不同 token budget 下的渲染大小
...
- **[`live_hit_rate.svg`](../../whale-pod/bench/results/live_hit_rate.svg)** — 实测命中率 vs 离线预测（run 1）；如果 `--only prune` 也会展示 prune 恢复曲线
- **[`live_tokens_split.svg`](../../whale-pod/bench/results/live_tokens_split.svg)** — 单个 session 的 cached vs fresh 堆叠图
- **[`live_prune.svg`](../../whale-pod/bench/results/live_prune.svg)** — prune 窗口下的命中率恢复曲线
- **[`live_provider_affinity.svg`](live_provider_affinity.svg)** — Pin vs Unpin A/B 测试（仅 `OpenRouter`，需运行 `--only pin-check` 才会生成）

`--reasoning-strip` 三种策略（never/tool/always）各跑一次后，用 `bench/reasoning_compare.py` 生成横向对比图：

- **[`reasoning_modes_hit_rate.svg`](../../whale-pod/bench/results/reasoning_modes_hit_rate.svg)** — 三种策略的 session 命中率中位数
- **[`reasoning_modes_per_request.svg`](../../whale-pod/bench/results/reasoning_modes_per_request.svg)** — 三种策略的 per-request 命中率折线
- **[`reasoning_modes_tokens.svg`](../../whale-pod/bench/results/reasoning_modes_tokens.svg)** — 三种策略的 cached vs fresh 堆叠图
...
- **[`live_hit_rate_band.svg`](../../whale-pod/bench/results/live_hit_rate_band.svg)** — 22 session 的 per-request median + P10/P90 带宽
- **[`turns_comparison.svg`](../../whale-pod/bench/results/turns_comparison.svg)** — 三种对话长度的命中率对比
- **[`mae_by_length.svg`](../../whale-pod/bench/results/mae_by_length.svg)** — 离线预测误差随对话长度的变化
- **[`prompt_tokens_by_length.svg`](../../whale-pod/bench/results/prompt_tokens_by_length.svg)** — 三种长度的 cached vs fresh 堆叠图
- **[`reasoning_tokens_by_mode.svg`](../../whale-pod/bench/results/reasoning_tokens_by_mode.svg)** — 三种策略每 session 的 reasoning token 量
- **[`cumulative_cache_growth.svg`](../../whale-pod/bench/results/cumulative_cache_growth.svg)** — 单 session 内缓存/新鲜 token 的累积曲线
...
![Reusable prefix per request, by context design](../../whale-pod/bench/results/reusable_prefix.svg)
...
![Session prompt tokens: reusable prefix vs freshly billed](../../whale-pod/bench/results/prompt_tokens_split.svg)

堆叠图清晰地看到：as-built 的柱子最矮（总 token 最少），且绿色（可复用）比例最高。no-ledger 柱子更高（更贵），three-zone 灰色（新鲜）部分最多（最贵）。

![Session prompt cost](../../whale-pod/bench/results/prompt_cost.svg)

成本对比：as-built **$0.020**，no-ledger **$0.025**（+23%），three-zone **$0.053**（+162%）。rolling-summary 的柱子矮是因为它压缩了上下文所以 token 总量少，但这是以牺牲信息为代价换来的。

![Prune recovery: 45k window forced](../../whale-pod/bench/results/prune_recovery.svg)
...
![Per-request cache hit rate: median with P10/P90 band](../../whale-pod/bench/results/live_hit_rate_band.svg)
...
![Session cache hit rate by conversation length](../../whale-pod/bench/results/turns_comparison.svg)

![Live prompt tokens by conversation length](../../whale-pod/bench/results/prompt_tokens_by_length.svg)

![Offline prediction vs server: mean absolute error](../../whale-pod/bench/results/mae_by_length.svg)
...
![Session cache hit rate by reasoning echo policy](../../whale-pod/bench/results/reasoning_modes_hit_rate.svg)

![Per-request cache hit rate, by reasoning echo policy](../../whale-pod/bench/results/reasoning_modes_per_request.svg)

![Prompt tokens by reasoning echo policy: cached vs billed fresh](../../whale-pod/bench/results/reasoning_modes_tokens.svg)

![Reasoning tokens echoed back per session, by policy](../../whale-pod/bench/results/reasoning_tokens_by_mode.svg)
...
![Cumulative prompt tokens over a 12-turn session](../../whale-pod/bench/results/cumulative_cache_growth.svg)
...
![Measured prefix-cache hit rate, live](../../whale-pod/bench/results/live_hit_rate.svg)
...
![Live session prompt tokens: served from cache vs billed fresh](../../whale-pod/bench/results/live_tokens_split.svg)
...
aggregate 的细粒度版本是 **[`live_hit_rate_band.svg`](../../whale-pod/bench/results/live_hit_rate_band.svg)**（[§6.2](#62-在线实测-12-轮对话2026-08-06deepseek-官方-api)）：逐请求画出 median 线并带 P10/P90 带宽。读法：
...
柱子是三种 `--reasoning-strip` 策略的命中率中位数。**回放越多命中率越高，但柱子的"高度"和它的成本不成正比**——高度只反映命中率，成本要结合 [`reasoning_modes_tokens.svg`](../../whale-pod/bench/results/reasoning_modes_tokens.svg) 里的柱高（总 prompt 量）和 [`reasoning_tokens_by_mode.svg`](../../whale-pod/bench/results/reasoning_tokens_by_mode.svg)（每 session 回放的思考 token）一起看。命中率最高（always）的方案把 prompt 总量放大了 1.55 倍，正是 [§3.4](#34-reasoning-控制回放-默认只保留工具回合的思考) 说"不值得"的做法。
