...
报告涵盖两条互补轨道的完整流程：离线预测（`bench/validate.py`，零网络零 API key）和在线实测（`bench/live_acceptance.py`，真实服务器），以及两个维度的评价指标——可复用前缀份额（reusable prefix ratio）和命中率（cache hit rate）。全文用 22 个独立 session 和三种对话长度的数据来论证设计的稳健性。
...
- **[`reusable_prefix_small_window.svg`](../../whale-pod/bench/results/reusable_prefix_small_window.svg)** — 同上，52k 窗口（强迫 prune）
...
- **[`prompt_cost_small_window.svg`](../../whale-pod/bench/results/prompt_cost_small_window.svg)** — 同上，52k 窗口
- **[`prune_recovery.svg`](../../whale-pod/bench/results/prune_recovery.svg)** — as-built 在 52k 窗口下，prune 后前缀恢复曲线（▼ 标记 prune 点）
...
- **[`live_provider_affinity.svg`](live_provider_affinity.svg)** — Pin vs Unpin A/B 测试（仅 `OpenRouter`）
...
![Prune recovery: 52k window forced](../../whale-pod/bench/results/prune_recovery.svg)

在小窗口下，prune 后的前缀恢复是瞬时的：一次 prune 掉到 ~0%（红色虚线标出），下一个请求立即恢复到 ~90%，session 整体仍保持 88.8% 的可复用比。这就是 prune 的全部代价。

### 6.2 在线实测 — 12 轮对话（2026-08-05，DeepSeek 官方 API）
...
22 次独立 session，命中率中位数 **94.7%**。P10=93.7%，P90=95.3%，跨度仅 1.6 点——评测结果具有统计学意义上的一致性。预热曲线显示：前 5 个请求建立缓存后，后续 32 个请求在 89–99.9% 之间维持，34 of 37 个请求超过 89%。离线预测 MAE 为 5.5 点（+tokenizer/encoder 改进前为 7.0）。
...
2. **不同长度对话均稳健**：6 轮 91.7%，12 轮 94.7%，18 轮 95.0%——对话越长缓存效果越好，12 轮是性价比甜点。
3. **离线预测精度的提升证明改进有效**：用官方 V4 BPE tokenizer 替代 cl100k_base/heuristic，用官方 `encode_messages` 替代手搓 `json.dumps`，MAE 从 7.0 降到 5.5 点。
4. **架构设计经得起检验**：两区存储 + ledger + reasoning 剥离 + compaction 七项决策的叠加效果在真实场景中持续兑现 90%+ 缓存命中率，不靠运气，不靠调参，靠的是字节级前缀稳定性的系统性保证。
