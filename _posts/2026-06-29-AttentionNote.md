...
本文对几种主流注意力机制做了统一风格的最小实现，每段代码都可**直接运行**。涵盖：

- **缩放点积注意力（SDPA）**：注意力计算的最小单元，由 2017 年《Attention Is All You Need》提出，在点积注意力基础上加入 $\frac{1}{\sqrt{d_k}}$ 缩放。注意力机制本身最早可追溯到 2014 年 Bahdanau 等人的**加性注意力**，但 Q/K/V 框架与缩放点积形式均来自 Transformer。
...
### 4. GQA 段落里的公式变量缺失（渲染丢了）

原文这段 LaTeX 没渲染出来，读起来是断的：

> 将所有 Head 分为 \_\_ 个组（\_\_ 可以整除 \_\_）……当 \_\_ 时就是 MHA，\_\_ 时就是 MQA，当 \_\_ 时，它只将 KV Cache 压缩到 \_\_ ……
...
需要的话，我可以再补上各机制的伪代码/张量维度对照，或加一段 MLA 中"矩阵吸收（matrix absorption）"与 RoPE 兼容性（解耦 RoPE）的技术细节——这部分是 MLA 实现中最容易踩坑的地方。