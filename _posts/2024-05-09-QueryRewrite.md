...
我们早期用SimRank++算法验证了构图方式两个优化点的可行性，SimRank++算法是一种同构信息网络中的相似度量算法，它的思想是：如果两个用户相似，则与这两个用户相关联的物品也类似；如果两个物品类似，则与这两个物品相关联的用户也类似。该算法的优点是可以使用Spark进行大规模全局优化，并且边权重可以根据需要调整。优化构图后人工评测SimRank++优化前后查询改写数据量提升了约30%，同时准确率从72%提升到83%。
...
#### 2.1.3 基于语义向量挖掘
在word2vec后，Embedding的思想迅速从NLP领域扩散到几乎所有机器学习的领域，号称“万物皆可Embedding”，只要是一个序列问题均可以从上下文的角度表示其中的节点。此外，Embedding在数据稀疏性表示上的优势也有利于后续深度学习的探索。将Query Embedding到低维语义空间，通过计算Embedding间的相似度查找相关词，在挖掘相似词的任务中是常见且易于实践的挖掘方法。除了简单的在用户评论等语料上训练大规模词向量外，在实践中还尝试了以下两种构建上下文的方法：

通过Query召回商户构建Doc2Vec：通过Query召回或点击的商户作为上下文训练Embedding表征Query。由于美团场景下同一商户提供的服务、商品繁多，该方法在没有考虑Query本身类目意图的情况下，噪声比较大。
通过用户Session构建改写序列：通过一个Session序列作为上下文训练Embedding表征Query。该方法的优点是有效的利用了用户自行换词的限制条件，挖掘覆盖率和准确率都相对更高。

设计不同的上下文结构得到Embedding后，为了进一步提高准确率后续的基本的步骤是：
① 训练语料通过分词后，利用fastText训练Query的词向量，fastText训练时考虑了字级别的Ngram特征，可以将未登录Query的字、词Embedding进行简单求和或求平均，解决OOV（Out-Of-Vocabulary）的问题；
② 在目标词表中，用词向量表示该词；
③ 利用LSH，查找向量cosine相似度高于一定阈值候选词，或使用DSSM双塔模型[7]，通过有监督训练提高精度；④ XGBoost结合特征工程进一步过滤。
...
实际服务中可能未必能使用 LLM，由于内部 LLM 产品的推理能力有限，借助 LLM 可能引入更多非预期的幻觉和其他不确定性。加上架构师的性能要求，内部 LLM 服务的响应速度会严重影响搜索阶段的时间开销，因此基于 LLM 的改写（暂时）不在本节说明 —— 因为这是在记录实际操作。

加上内部 RAG 服务刚刚搭建完成，各接入部门均没有对用户日志/行为做妥善的收集和清洗，数据飞轮也没有建立起来，导致整个服务在一个非常彻底的冷启动状态。依赖大量用户业务数据的机器学习方法均没有办法启用，这也是实际操作中非常需要马上解决的一个问题。

综上，本节探索以上约束场景下基于统计学方法的高效低开销，又能有实际改进效果的改写方案。

## 删除低权重token

完成分词后，在原始知识库中计算各token权重（如以IDF作为指标），同时以原始query大致筛选出一部分目标文档。将低权重token去除，组成新的query再在更相关的目标文档进行二次检索，重新获得一组结果。简单判断这组结果的相似度是否高于改写阈值，确定是否改写。



![Rewrite-Filtering](/images/illustration/2024-05-09/QueryRewrite_Filtering_240612.png "Rewrite - Query Token Filtering")
