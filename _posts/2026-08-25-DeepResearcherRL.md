...
1. 比较完整的多轮 Agent Loop，检测 action token → 调用工具 → 拼接 observation → 继续生成
2. 插件式工具系统，注册即用
3. 带一个工具服务器，基于 HTTP 服务器，解耦推理与工具调用
...
每当模型更新后，收集样本的旧策略$π_old$ 也会更新，每次 iteration 的 baseline 都在漂移。
